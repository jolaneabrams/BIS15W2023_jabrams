...
```r
sydbchs <-read_csv(here("lab8", "data", "sydneybeaches.csv")) %>% janitor::clean_names()
```
...
```r
naniar::miss_var_summary(sydbchs)
```
...
They're tidy: each variable has its own column, each observation has its own row, and each value has its own cell. It's in long format.  
...
```r
avg_cocci_wide <- avg_cocci %>%
  pivot_wider(names_from = year,
               values_from = mean_enterococci_cfu_100ml
              ) %>%
  group_by(site) %>% 
  summarize(across(contains("20"), mean, na.rm = T))
avg_cocci_wide
```
