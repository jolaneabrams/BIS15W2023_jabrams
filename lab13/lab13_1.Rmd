...
```{r}
names(spider_list[[18]])
```

```{r}
glimpse(spider_list[[18]])
```
```{r}
str(spider_list[[18]])
```
```{r}
naniar::miss_var_summary((spider_list[[18]]))
```
...
`inner_join`  
Where did customers 3 and 4 go? Only gives output where observations perfectly in common
```{r}
...
`left_join`
Where did customer 4 go? Notice the missing data for customer 3. Excludes missing data from other table
```{r}
...
`full_join` joins everything - easier to deal with missing data via filter rather than lose data inadvertently
```{r}
