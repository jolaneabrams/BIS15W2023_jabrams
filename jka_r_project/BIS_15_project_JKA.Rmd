...
# See the culprit that is breaking your ylim:
danx %>% 
  filter(avg_brood > 18)
```
...
ggplot(danx, aes(x = anxiety_total, y = avg_brood, fill = anxiety_total)) +
  ...
  facet_grid(culture ~ .,
             ...
             space = "free", 
             labeller = labeller(culture = c(`0` = "Euro-Australian", `1` = "Malaysian")))+ 
#Hannah Edit: change y limit to 21 instead of 18
...
```{r}
# Not sure you you still need this, didnt want to delet :) 
#  - Hannah
...
```{r}
ptsd <- ptsd %>%
  mutate(rrs = rrs1 + rrs2 + rrs3 + rrs4 + rrs5 + rrs6 + rrs7 + rrs8 + rrs9 + rrs10) %>% 
  filter(traumacoding == 0 - 6)
