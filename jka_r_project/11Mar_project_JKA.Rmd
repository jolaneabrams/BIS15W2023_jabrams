...
```{r}
ptsd$participantnumber <- as.factor(ptsd$participantnumber)
ptsd$group <- as.factor(ptsd$group) 
ptsd$traumacoding <- as.factor(ptsd$traumacoding)
ptsd$pts_dtotal<- as.factor(ptsd$pts_dtotal) 
ptsd$had_sdep<- as.factor(ptsd$had_sdep) 
ptsd$had_sanx <- as.factor(ptsd$had_sanx) 
ptsd$age <- as.factor(ptsd$age) 
ptsd$gender <- as.factor(ptsd$gender) 
ptsd$education <- as.factor(ptsd$education) 
ptsd$ethnicity <- as.factor(ptsd$ethnicity) 
ptsd$relgion <- as.factor(ptsd$relgion) 
ptsd$religioncode <- as.factor(ptsd$religioncode) 
...
```{r}
 ptsd_sum_sep <- left_join(ptsd_sum_sep, ptsd_long %>% 
                             select(participantnumber, had_sdep), by = "participantnumber")
```
...
unique_participants <- unique(ptsd_sum_sep$participantnumber)
# Plot the scatter plot
ggplot(ptsd_sum_sep, aes(x = index_t, y = as.numeric(rrs_sum), color = index_t)) +
  geom_point(size = 0.75) +
  scale_color_manual(values = my_palette, 
                     labels = c("Accident/Injury/Illness",
                                "Non-sexual Abuse",
                                "Sexual Abuse",
                                "Witnessing Death",
...
```{r}
 ptsd_long%>% 
  group_by(had_sdep, rrs_sum) %>% 
  ggplot(aes(x = had_sdep, y = rrs_sums, color = had_sdep)) +
  geom_point(alpha = 0.5) +
...
ptsd_long %>% 
  filter(test=="rrs") %>% 
  select(participantnumber, test, q, score)
  #rowwise() %>% 
  #mutate(sum_rrs=sum(c_across(score(starts_with("rrs")), na.rm = T))) 
```
